...
import {openModal, closeModal,preload,fetchToState} from 'react-isomorphic-tools'
...
@connect(state => ({
discountsCategory:state.getIn(['discountsCategory']),
...
getWidthHeight= (sz)=>{
  let wh;
  switch (sz){
      // case '1':wh={w:6,h:6};break;
      case '2':wh={w:2,h:2};break;
      // case '3':wh={w:2,h:2};break;
      // case '4':wh={w:2,h:2};break;
          default:
          return {w:2,h:2}
  }
  return wh
...
renderLayoutLg= ()=>{
    const {discountsCategory,categoryItems,currentLayout}=this.props
...
renderLayoutMd= ()=>{
    const {discountsCategory,categoryItems,currentLanguage,discountFilter,currentLayout}=this.props
    console.log(currentLayout.get('name'));
  return(
    <ReactGridLayout
 ...
 categoryItems.map((item,i)=>{
    ...
    return(
      ...
      >
        ...
        {/* <p>layout settings i {JSON.parse(item.get('layout_settings')).lg.i}</p>
        // <p>item id =>{item.get('id')}</p> */}
    ...
    return(
      <div className='admin-grid-wrapper'>
        <div className='admin-grid'>
          {discountsCategories.map(category=>{return(
            <div
              key={category.get('id')}
